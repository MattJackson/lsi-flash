...
### Fixed
- sbr read: `i2c_init` now unlocks the chip's diagnostic registers (WRSEQ sequence + DIAG_RW_ENABLE) before any DCR access. Without the unlock, DCR reads were stale and EEPROM addr/type auto-detect picked 0x50/type-2 instead of the correct 0x54/type-1 (observed on dev-1 2026-05-29 → "EEPROM did not ACK address W"). Cites `src/sbr/i2c.rs::unlock_diag` (lsirec.c:125-160).
- sbr read now calls i2c_init to auto-detect EEPROM address/type (0x54/16-bit on Dell H200) instead of hardcoding 0x50/8-bit. Cites: `src/sbr/i2c.rs` DCR helpers (`dcr_read32`/`dcr_write32`) operating on live BAR1 via registers at 0x3c/0x38 (lsirec.c:40-41, 113-122); `i2c_init` reads DCR_SBR_CONFIG to determine addr/type (lsirec.c:498-524); reset loop bounded to 100k iterations to prevent infinite hang on wedged controller.
